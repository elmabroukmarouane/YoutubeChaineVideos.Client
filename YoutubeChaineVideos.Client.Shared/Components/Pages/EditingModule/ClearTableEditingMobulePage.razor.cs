...
{
    public partial class ClearTableEditingMobulePage : ComponentBase
    {
        ...
        private async Task Ok()
        {
            ...
            var messageStatusVideo = await VideoService!.Truncate($"{BaseSettingsApp?.BaseUrlApiWebHttp}Video/Truncate?isSqlite={IsSqlite}", Token);

            if (Success || Errors.Length <= 0)
            {
                IsLoading = true;
                if (messageStatus is not null && messageStatusVideo is not null)
                {
                    IsLoading = false;
